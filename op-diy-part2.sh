...
sed -i 's/ln_start_bin "$(first_type $(config_t_get global_app xray_file) xray)" xray $log_file -config="$config_file"/ln_start_bin "$(first_type $(config_t_get global_app xray_file) v2ray)" v2ray $log_file -config="$config_file"/' feeds/lienol/luci-app-passwall/root/usr/share/passwall/app.sh
